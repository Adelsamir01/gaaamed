...
import type { BoardTile, GameLogEntry, GameState, Player } from "@bank-el7az/shared";
import type { PlayerStats, StatsSnapshot, TileStats } from "@bank-el7az/shared";
import {
  Activity,
  Banknote,
  BarChart3,
...
function GameApp() {
  ...
  const [invitedRoomCode, setInvitedRoomCode] = useState<string | null>(() => getInviteRoomCode());
  ...
  useEffect(() => {
    if (state?.roomCode) {
      setInvitedRoomCode(state.roomCode);
      replaceInviteRoomInUrl(state.roomCode);
    }
  }, [state?.roomCode]);
  ...
  if (!state) {
    return (
      <HomeScreen
        ...
        clearInviteRoom={clearInviteRoom}
        status={game.status}
        error={game.error}
      />
    );
...
function HomeScreen({
  ...
  const [name, setName] = useState(localStorage.getItem("bank-el7az-name") ?? "");
  ...
  function submitCreate(event: FormEvent) {
    ...
    localStorage.setItem("bank-el7az-name", name);
  ...
  function submitJoin(event: FormEvent) {
    ...
    localStorage.setItem("bank-el7az-name", name);
    void enterImmersiveMode();
    joinRoom(invitedRoomCode ?? roomCode, name);
  }

  const isInviteJoin = Boolean(invitedRoomCode);
    ...
    <main className="app-shell join-shell">
      <section className="join-panel">
        <div className="brand-lockup">
          ...
          <div>
            <h1>بنك الحظ</h1>
            <p>لعبة محافظات مصرية أونلاين</p>
          </div>
        </div>

        {isInviteJoin ? (
          <form className="join-form invite-join-form" onSubmit={submitJoin}>
            <div className="invite-room-banner">
              <span>دعوة لأوضة</span>
              <strong className="room-code" dir="ltr">
                {invitedRoomCode}
              </strong>
            </div>
            <label>
            ...
            <button type="button" className="secondary-action" onClick={clearInviteRoom}>
              <RefreshCcw size={18} />
              اختار أوضة تانية
            </button>
          </form>
          ...
          <>
            <form className="join-form" onSubmit={submitCreate}>
              <label>
                اسم اللاعب
                <input
                  value={name}
            ...
            <form className="join-form compact" onSubmit={submitJoin}>
              <label>
                كود الأوضة
                <input
        ...
        <ConnectionLine status={status} error={error} />
      </section>
    </main>
  );
}
...
function LobbyScreen({
  ...
  const inviteUrl = makeInviteUrl(state.roomCode);

  async function copyCode() {
    try {
      await navigator.clipboard?.writeText(inviteUrl);
      setInviteStatus("لينك الدعوة اتنسخ");
      window.setTimeout(() => setInviteStatus(""), 2200);
    } catch {
      setInviteStatus("انسخ اللينك من الخانة");
    }
  }
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "بنك الحظ",
          text: `ادخل أوضة ${state.roomCode} في بنك الحظ`,
          url: inviteUrl
        });
        return;
      }
    ...
    <main className="app-shell lobby-shell">
      <section className="lobby-panel">
        <div className="lobby-heading">
          <div>
            <span className="eyebrow">كود الأوضة</span>
            <h1 className="room-code" dir="ltr">
              {state.roomCode}
            </h1>
          </div>
          <button className="icon-button" onClick={copyCode} aria-label="انسخ لينك الدعوة">
            <Copy size={20} />
          </button>
        </div>
        ...
        <div className="invite-link-card">
          <label>
            لينك الدعوة
            <input className="invite-link-input" dir="ltr" value={inviteUrl} readOnly onFocus={(event) => event.target.select()} />
          </label>
          ...
          <span>{inviteStatus || "اللي يفتح اللينك هيكتب اسمه بس ويدخل الأوضة."}</span>
        </div>

        <div className="lobby-note">ابعت اللينك لصاحبك. أول ما يبقى فيه لاعبين، صاحب الأوضة يقدر يبدأ.</div>
          ...
          {state.players.map((player) => (
            <article className="player-row" key={player.id}>
              ...
              <div>
                ...
                <span>{player.id === state.hostId ? "صاحب الأوضة" : player.connected ? "جاهز" : "فاصل"}</span>
              </div>
            </article>
          ))}
        </div>

        <button className="primary-action" disabled={!canStart} onClick={startImmersiveGame}>
          <Play size={18} />
          ابدأ اللعب
        </button>

        <ConnectionLine status={status} error={error ?? (!isHost ? "مستني صاحب الأوضة يبدأ." : null)} />
      </section>
    </main>
  );
}
...
function makeInviteUrl(roomCode: string): string {
  const url = new URL("/", window.location.origin);
  url.searchParams.set("room", roomCode);
  return url.toString();
}

function replaceInviteRoomInUrl(roomCode: string): void {
...
function clearInviteRoomFromUrl(): void {
  const nextUrl = new URL(window.location.href);
